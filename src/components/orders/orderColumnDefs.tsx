  ...
  return [
      ...
      id: "produto", name: "Produto", enabled: true, alwaysVisible: true, render: (pedido: any) => (
        <div className="flex flex-col space-y-1">
          {pedido.items?.map((it: any, idx: number) => (
            <div key={it.id ?? `item-${idx}`} className="flex items-center space-x-1 min-h-[15px] py-0.9">
              <img
                src={((idx === 0 ? (pedido.imageUrl || it?.imageUrl) : it?.imageUrl) || '/placeholder.svg')}
                alt={(idx === 0 ? (pedido.productTitle || it?.name || 'Produto') : (it?.name || 'Produto'))}
              ...
              <div className="min-w-0 flex-none w-[82%]">
                <div className={`text-sm font-medium text-gray-900 ${pedido.totalQuantity >= 2 ? 'font-bold' : ''}`}>
                  {(() => {
                    const rawTitle: string = idx === 0 ? (pedido.productTitle || it?.name || 'Produto') : (it?.name || 'Produto');
                    const displayTitle: string = rawTitle.length > 40 ? rawTitle.slice(0, 40) + '..' : rawTitle;
                    const link: string | null = (it?.permalink ?? null);
                    if (link) {
                      return (
                        <a
                ...
                {it?.variationLabel && (
                  <div className="text-xs text-gray-500">{it.variationLabel}</div>
                )}
              </div>
            </div>
          ))}
      ...
      id: "itens", name: "Itens", enabled: true, alwaysVisible: true, render: (pedido: any) => (
        <div className="flex flex-col items-center space-y-1">
          {pedido.items?.map((item: any, index: number) => (
            <div key={item.id ?? `item-${index}`} className="min-h-10 py-0.5 flex items-center justify-center w-full">
              <span
                className={`inline-flex items-center justify-center h-6 min-w-6 rounded-md px-2 text-sm md:text-base border ${pedido.totalQuantity >= 2 ? 'text-purple-600 border-purple-600 bg-purple-600/10' : 'text-gray-700 border-gray-300'}`}
                title={`Qtd: ${item.quantity}`}
              >
                {item.quantity}
              </span>
            </div>
    ...
    },
    {
      id: "cliente", name: "Cliente", enabled: false, render: (pedido: any) => {
        const name = String(pedido?.customerName ?? "");
    ...
    },
    {
      id: "valor", name: "Valor do Pedido", enabled: true, render: (pedido: any) => (
        <span className="text-gray-900 font-semibold">{pedido.totalAmount?.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}</span>
      )
    },
    {
      id: "tipoEnvio", name: "Tipo de Envio", enabled: true, alwaysVisible: true, render: (pedido: any) => {
        ...
        return (
          <div className="flex flex-col items-center justify-center gap-1 text-center">
            {(() => {
              const lbl = mapTipoEnvioLabel(pedido.shippingType);
    ...
    },
    {
      id: "marketplace", name: "Marketplace", enabled: true, render: (pedido: any) => (
        <div className="flex flex-col leading-tight">
          ...
          <span className="text-xs text-gray-500 break-all">{String(pedido.platformId ?? '')}</span>
        </div>
      )
    },
    {
              ...
              <div className="flex items-center space-x-2 mt-1">
                <Tooltip>
                  <TooltipTrigger asChild>
                    <StickyNote className={`h-4 w-4 ${pedido.pickingListPrinted ? 'text-primary' : 'text-gray-300'}`} />
                  </TooltipTrigger>
                  <TooltipContent>
                    <p>{pedido.pickingListPrinted ? 'Lista de Separação Impressa' : 'Lista de Separação não impressa'}</p>
                  </TooltipContent>
                </Tooltip>
                <Tooltip>
                  <TooltipTrigger asChild>
                    <FileBadge className={`h-4 w-4 ${pedido.labelPrinted ? 'text-primary' : 'text-gray-300'}`} />
                  </TooltipTrigger>
                  <TooltipContent>
                    <p>{pedido.labelPrinted ? 'Etiqueta Impressa' : 'Etiqueta não impressa'}</p>
                  </TooltipContent>
                </Tooltip>
              </div>
            )}
    ...
    },
    {
      id: "margem", name: "Margem %", enabled: true, alwaysVisible: true, render: (pedido: any) => {
        ...
        const comissaoMarketplace = tn(f?.marketplaceFee);
        const impostosCalculados = tn(f?.taxAmount);
        const custoProdutosFixo = tn(f?.productCost);
        const custosExtras = tn(f?.extraCosts);
        const cupomFixo = tn(f?.couponAmount);
        const freteCusto = tn(f?.shippingCost);
        const freteRecebidoLiquido = tn(f?.shippingNetReceived ?? (tn(f?.shippingReceived) - tn(f?.shippingFeeBuyer)));
