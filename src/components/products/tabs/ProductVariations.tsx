...
export function ProdutosVariacoes() {
  ...
  const { variationGroups, loading, deleteVariationGroup } = useVariations();
  ...
  const filteredVariations = (list || [])
    ...
    .filter(group => {
      ...
      const sku = String(group?.sku ?? "").toLowerCase();
  ...
  return (
    <div className="space-y-6">
      ...
      <VariationsAccordion
        ...
        onDeleteVariationGroup={async (productId) => {
          const found = list.find((p) => p.id === productId);
          setSingleDeleteTarget({ id: productId, name: found?.name || "Grupo de variações" });
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Excluir grupo de variações</AlertDialogTitle>
            <AlertDialogDescription>
              Você está prestes a excluir <strong className="text-foreground">&quot;{singleDeleteTarget?.name || "-"}&quot;</strong>. Deseja continuar?
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>Cancelar</AlertDialogCancel>
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Esta ação não poderá ser desfeita. Confirma excluir este grupo de variações?
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            ...
            <AlertDialogAction
              ...
              onClick={async () => {
                ...
                try {
                  await deleteVariationGroup(singleDeleteTarget.id);
