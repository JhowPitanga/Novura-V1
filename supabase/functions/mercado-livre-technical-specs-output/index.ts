...
serve(async (req) => {
  ...
  try {
    ...
    let resp = await fetch(`https://api.mercadolibre.com/categories/${categoryId}/technical_specs/output`, { headers: { Authorization: `Bearer ${accessToken}`, Accept: "application/json" } });
    if (resp.status === 401 || resp.status === 403) {
      ...
      resp = await fetch(`https://api.mercadolibre.com/categories/${categoryId}/technical_specs/output`, { headers: { Authorization: `Bearer ${tokenJson.access_token}`, Accept: "application/json" } });
    }
