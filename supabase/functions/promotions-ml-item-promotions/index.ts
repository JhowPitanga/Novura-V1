/**
 * promotions-ml-item-promotions
 * Returns all ML promotions an item participates in (360° view).
 * GET /seller-promotions/items/{item_id} (Mercado Livre only)
 *
 * Body: { integrationId: string, marketplaceItemId: string }
 */
...
serve(async (req) => {
  ...
  const { integrationId, marketplaceItemId } = body;
  if (!integrationId || !marketplaceItemId) {
    return jsonResponse({ error: "integrationId and marketplaceItemId required" }, 400);
  }
  ...
  try {
    ...
    if (!adapter.getMlItemPromotions) {
      throw new ProviderUnsupportedError("getMlItemPromotions", marketplaceName);
    }

    const promotions = await adapter.getMlItemPromotions(marketplaceItemId);
    return jsonResponse({ ok: true, promotions });
  } catch (e: any) {
    if (e instanceof ProviderUnsupportedError) {
      return jsonResponse({ ok: false, error: e.message }, 422);
    }
    console.error("promotions-ml-item-promotions error:", e);
