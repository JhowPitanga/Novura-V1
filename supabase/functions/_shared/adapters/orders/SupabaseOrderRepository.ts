...
type OrderRow = {
  ...
  readonly order_items: ReadonlyArray<{ readonly id: string; readonly product_id: string | null; readonly marketplace_item_id: string | null; readonly quantity: number | null }> | null;
};
...
export class SupabaseOrderRepository implements IOrderRepository {
  ...
  private mapOrderRow(row: OrderRow): OrderRecord {
    ...
    return { id: row.id, organizationId: row.organization_id, marketplace: row.marketplace, marketplaceOrderId: row.marketplace_order_id, currentStatus: row.status, marketplaceSignals: signals, items };
  }
}
