...
import { mapShopeeQuality, shopeeNumericQualityScore } from './shared/qualityMapping.ts';
import { parseShopeeDimensions } from './shared/dimensionsParse.ts';
import { resolveCanonicalFees } from './shared/feesResolve.ts';
import { extractShopeeImageUrlList } from './prepareAdapterPayload.ts';
...
function normalizeShopeeItem(payload: unknown, ctx: AdapterContext): NormalizedListing {
  ...
  const rowModels = Array.isArray(raw.variations) ? (raw.variations as ShopeeModel[]) : [];
  const models =
    (Array.isArray(shopee.model_list) && shopee.model_list.length > 0)
      ? shopee.model_list
      : rowModels;
  ...
  // Listing core
  // -------------------------------------------------------------------------
  const imageUrlList = extractShopeeImageUrlList(raw);
  const thumbnail = imageUrlList[0] ?? base.image?.image_url_list?.[0] ?? null;
  ...
  const listing: CanonicalListing = {
    ...
    has_variations: models.length > 0 && (base.has_model === true || models.length > 1),
  ...
  const variations: CanonicalVariation[] = models.map((m, i) => {
    ...
    return {
      ...
      sold_quantity: 0,
      image_url: m.model_image_url ?? m.image?.image_url ?? null,
  ...
  // Pictures
  // -------------------------------------------------------------------------
  const pictures: CanonicalPicture[] = imageUrlList.map((url, idx) => ({
  ...
  // One picture row per model/variation (variation_id resolved to UUID on upsert)
  for (const m of models) {
    const url = m.model_image_url ?? m.image?.image_url ?? null;
    const marketplaceVarId = String(m.model_id ?? '');
    if (!url || !marketplaceVarId) continue;
    pictures.push({
      external_picture_id: marketplaceVarId,
      url,
      secure_url: null,
      position: 0,
      is_video: false,
  ...
  // Quality
  // -------------------------------------------------------------------------
  const rawPd = raw['performance_data'] as { quality_level?: string | number } | undefined;
  const shopeeLevelRaw = diagnosis.quality_level ?? rawPd?.quality_level;
  const shopeeLevelNum =
    typeof shopeeLevelRaw === 'number' ? shopeeLevelRaw : Number(shopeeLevelRaw);
  const quality: CanonicalQuality = {
    quality_score: Number.isFinite(shopeeLevelNum)
      ? shopeeNumericQualityScore(shopeeLevelNum)
      : null,
    quality_level: mapShopeeQuality(shopeeLevelRaw),
