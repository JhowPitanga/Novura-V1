/**
 * promotions-list-flash-slots
 * Returns available Shopee flash-sale time slots with item criteria.
 * Throws ProviderUnsupportedError for non-Shopee integrations (returns empty list gracefully).
...
serve(async (req) => {
  ...
  if (!integrationId) return jsonResponse({ error: "integrationId required" }, 400);
  ...
  try {
    ...
    if (!adapter.listFlashSaleSlots) {
      return jsonResponse({ ok: true, slots: [] });
    }

    const slots = await adapter.listFlashSaleSlots();
    return jsonResponse({ ok: true, slots });
  } catch (e: any) {
    if (e instanceof ProviderUnsupportedError) {
      return jsonResponse({ ok: true, slots: [] });
    }
    console.error("promotions-list-flash-slots error:", e);
