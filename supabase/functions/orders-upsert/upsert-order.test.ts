/**
 * Unit tests for orders-upsert (upsertOrder). Run with: deno test -A upsert-order.test.ts
 * Uses a mock Supabase client that returns configured results.
 */

import { assertEquals } from "https://deno.land/std@0.224.0/assert/mod.ts";
import type { NormalizedOrder, UpsertOrderInput } from "../_shared/domain/orders/orders-types.ts";
...
function createMockClient(initialOrder: { id: string; marketplace_status: string; status: string | null } | null) {
  ...
  return {
    mock: { from } as unknown as Parameters<typeof upsertOrder>[0],
    historyInserts,
  };
}
...
Deno.test("upsertOrder: inserts into order_status_history when status changes", async () => {
...
Deno.test("upsertOrder: created is false when order already existed", async () => {
...
Deno.test("upsertOrder: returns error when orders upsert fails", async () => {
  const from = (table: string) => {
    if (table === "orders") {
      return {
        select: () => ({
  ...
  const mock = { from } as unknown as Parameters<typeof upsertOrder>[0];
  const result = await upsertOrder(mock, {
    organization_id: "org-1",
    order: minimalOrder(),
    source: "sync",
  });
