...
import { UnmarkOrderLabelPrintedUseCase } from "../_shared/application/orders/UnmarkOrderLabelPrintedUseCase.ts";

/**
 * POST /unmark-labels-printed
 * Clears the label-printed flag on one or more orders and recalculates their status.
 * Mirrors mark-labels-printed in reverse.
 */
serve(async (req: Request) => {
  ...
  try {
    ...
    const unmarkPrinted = new UnmarkOrderLabelPrintedUseCase(orderRepo, recalculate);

    const results = await Promise.allSettled(
      (orderIds as string[]).map((orderId) =>
        unmarkPrinted.execute({ orderId, organizationId } as any)
      ),
    );
    ...
    if (errors.length > 0) {
      console.error("[unmark-labels-printed] Partial failures:", errors);
    }

    return jsonResponse({ statusChanges, processed: orderIds.length, errors });
  } catch (error) {
    console.error("[unmark-labels-printed] Error:", error);
