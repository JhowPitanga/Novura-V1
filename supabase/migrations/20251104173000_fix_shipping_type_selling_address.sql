...
-- Corrige registros antigos onde location_type = 'selling_address' foi marcado como 'flex'.
-- Se logistic_type for 'drop_off', corrigir diretamente para 'correios'; caso contrário, usar 'agencia' (tratado na próxima migração).
...
WHERE msd.location_type = 'selling_address'
  AND (msd.shipping_type IS NULL OR msd.shipping_type = 'flex')
...
-- Recalcula o array shipping_types em marketplace_items a partir da distribuição corrigida.
-- Apenas para itens do Mercado Livre afetados por 'selling_address'.
WITH agg AS (
  SELECT
    mi.organizations_id,
    mi.marketplace_name,
  ...
  WHERE mi.marketplace_name = 'Mercado Livre'
    AND msd.shipping_type IS NOT NULL
...
-- Atualiza o campo JSONB stock_distribution.shipping_types para refletir os tipos corrigidos.
WITH agg2 AS (
  SELECT
    mi.organizations_id,
    mi.marketplace_name,
