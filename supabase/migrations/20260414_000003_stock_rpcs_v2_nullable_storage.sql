-- Make p_storage_id optional (DEFAULT NULL) on stock RPCs v2.
-- When NULL, fallback to orders.storage_id then to the org's default active physical storage.
-- This allows safe calls even when warehouse config hasn't been resolved yet.
...
DECLARE
  ...
  v_skipped     integer := 0;
BEGIN
  -- Resolve org, pack, and storage_id from the order row.
  SELECT o.organization_id, o.pack_id, o.storage_id
  INTO v_org_id, v_pack_id, v_storage_id
