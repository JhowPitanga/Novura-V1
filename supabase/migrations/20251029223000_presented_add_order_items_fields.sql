-- Adiciona colunas agregadas de itens (order_items) na view marketplace_orders_presented
-- Mantém colunas previamente adicionadas (shipping, payment, buyer) e evita valores NULL
...
WITH base AS (
  ...
  FROM base b
  LEFT JOIN LATERAL jsonb_array_elements(COALESCE(b.order_items, '[]'::jsonb)) AS i ON true
  GROUP BY b.id
), items_agg AS (
  SELECT
    b.id,
...
SELECT
  ...
  -- Novas colunas agregadas de itens
