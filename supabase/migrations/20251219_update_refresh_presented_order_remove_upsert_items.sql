...
BEGIN
  ...
  SELECT
    ...
    COALESCE(
      ...
      NULLIF(split_part(rec.data->'shipping'->'receiver_address'->'state'->>'id','-',2), ''),
  ...
  SELECT
    ...
    CASE
      ...
      WHEN jsonb_typeof(rec.order_items->0->'item'->'variation_id') = 'string' THEN CASE WHEN (rec.order_items->0->'item'->>'variation_id') ~ '^\\d+$' THEN (rec.order_items->0->'item'->>'variation_id')::bigint ELSE NULL END
      ELSE NULL
    ...
    CASE
      ...
      THEN (
        CASE
          WHEN rec.order_items->0->'item'->>'permalink' IS NOT NULL THEN rec.order_items->0->'item'->>'permalink'
          ELSE COALESCE(rec.order_items->0->>'permalink', NULL)
        END
      )
      ELSE NULL
  ...
  SELECT
    ...
    COALESCE(
      NULLIF(rec.shipments->0->'lead_time'->'estimated_delivery_limit'->>'date','')::timestamptz,
      NULLIF(rec.shipments->0->'lead_time'->'estimated_delivery_final'->>'date','')::timestamptz,
      NULLIF(rec.shipments->0->'shipping_option'->'estimated_delivery_limit'->>'date','')::timestamptz,
      NULLIF(rec.data->'shipping'->'estimated_delivery_limit'->>'date','')::timestamptz
    ...
    COALESCE(
      NULLIF(rec.shipments->0->'sla'->>'expected_date','')::timestamptz,
      NULLIF(rec.shipments->0->>'sla_expected_date','')::timestamptz,
      NULLIF(rec.shipments->0->'lead_time'->'estimated_delivery_limit'->>'date','')::timestamptz
    ) AS shipment_sla_expected_date,
    COALESCE(
      NULLIF(rec.shipments->0->'sla'->>'last_updated','')::timestamptz,
      NULLIF(rec.shipments->0->>'sla_last_updated','')::timestamptz,
      rec.last_updated
  ...
  v_label_cached := COALESCE((rec.labels->>'label_cached')::boolean, false);
  v_label_response_type := COALESCE(rec.labels->>'response_type', NULL);
  v_label_fetched_at := NULLIF(rec.labels->>'fetched_at','')::timestamp;
  v_label_size_bytes := COALESCE((rec.labels->>'size_bytes')::int, NULL);
  v_label_content_base64 := COALESCE(rec.labels->>'label_content_base64', NULL);
  v_label_content_type := COALESCE(rec.labels->>'content_type', NULL);
  v_label_pdf_base64 := COALESCE(rec.labels->>'label_pdf_base64', NULL);
  v_label_zpl2_base64 := COALESCE(rec.labels->>'label_zpl2_base64', NULL);
  ...
  v_is_full := COALESCE(v_is_full, false);
  v_is_cancelled := COALESCE(v_is_cancelled, false);
  v_is_refunded := COALESCE(v_is_refunded, false);
  v_is_returned := COALESCE(v_is_returned, false);
  ...
  SELECT has_unlinked_items INTO v_has_unlinked_items
  FROM public.marketplace_orders_presented_new
  WHERE id = rec.id
  LIMIT 1;
  IF v_has_unlinked_items IS NULL THEN
    v_has_unlinked_items := COALESCE(v_unlinked_items_count, 0) > 0;
  END IF;
  v_shipping_type := COALESCE(v_shipping_type, NULL);
  v_shipment_status := COALESCE(v_shipment_status, NULL);
  v_shipment_substatus := COALESCE(v_shipment_substatus, NULL);
  ...
  VALUES (
    rec.id, rec.organizations_id, rec.company_id, rec.marketplace_name, rec.marketplace_order_id, COALESCE(rec.status, NULL), rec.status_detail::text, COALESCE((rec.data->>'total_amount')::numeric, NULL),
    ...
    CASE
      WHEN rec.marketplace_name = 'Mercado Livre' THEN COALESCE(rec.data->'pack_id'->>'id', NULL)
      ELSE NULL
    END,
    ...
    COALESCE(v_unlinked_items_count, 0), COALESCE(v_has_unlinked_items, false), COALESCE(v_linked_products, '[]'::jsonb), rec.date_created, rec.last_updated, rec.last_synced_at, v_status_interno
  )
  ON CONFLICT (id) DO UPDATE SET
    organizations_id = EXCLUDED.organizations_id,
    company_id = EXCLUDED.company_id,
