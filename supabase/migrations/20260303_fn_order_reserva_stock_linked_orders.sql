-- Support orders table in fn_order_reserva_stock_linked: when p_order_id is from orders.id,
-- resolve org/company from orders + companies and first_item_id from order_items; on failure update orders.status.
...
DECLARE
  ...
  v_from_orders boolean := false;
BEGIN
  -- Try orders table first (frontend migration to orders)
  SELECT o.organization_id, o.marketplace_order_id, o.pack_id::text, o.marketplace
  INTO v_org_id, v_marketplace_order_id, v_pack_id_text, v_marketplace
  ...
  IF FOUND THEN
    ...
    LIMIT 1;
    SELECT oi.marketplace_item_id INTO v_first_item_id
    FROM public.order_items oi
    WHERE oi.order_id = p_order_id
    LIMIT 1;
  END IF;

  -- Fallback: legacy marketplace_orders_presented_new / marketplace_orders_raw
  ...
  LOOP
    ...
    IF v_permanent THEN
      IF COALESCE(v_item_id,'') = '' THEN
        IF v_from_orders THEN
          IF COALESCE(v_variation_id,'') <> '' THEN
            SELECT oi.marketplace_item_id INTO v_item_id
            FROM public.order_items oi
            WHERE oi.order_id = p_order_id
              AND (oi.marketplace_item_id = v_variation_id OR oi.variation_name = v_variation_id)
            LIMIT 1;
          END IF;
          IF COALESCE(v_item_id,'') = '' AND COALESCE(v_first_item_id,'') <> '' THEN
            v_item_id := v_first_item_id;
          ELSIF COALESCE(v_item_id,'') = '' THEN
            SELECT oi.marketplace_item_id INTO v_item_id
            FROM public.order_items oi
            WHERE oi.order_id = p_order_id
            LIMIT 1;
  ...
  IF v_failures > 0 THEN
    IF v_from_orders THEN
      UPDATE public.orders SET status = 'Sem estoque' WHERE id = p_order_id;
    ELSE
      UPDATE public.marketplace_orders_presented_new SET status_interno = 'Sem estoque' WHERE id = p_order_id;
    END IF;
