-- Hotfix: duplicate_product must set parent_id explicitly
-- products.parent_id has default gen_random_uuid(), which breaks UNICO constraint.
BEGIN;
...
BEGIN
  ...
  INSERT INTO public.products (
    organizations_id, company_id, user_id, type, parent_id,
  ...
  SELECT
    ...
    auth.uid(),
    type,
    CASE WHEN type = 'UNICO' THEN NULL ELSE parent_id END,
    name || ' - Copia',
    v_new_sku,
    description,
    category_id,
