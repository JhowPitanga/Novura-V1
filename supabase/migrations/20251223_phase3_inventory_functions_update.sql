...
BEGIN
  ...
  LOOP
    ...
    SELECT NULLIF(moi.linked_products,'')::uuid
    INTO v_product_id
    FROM public.marketplace_order_items moi
    WHERE moi.id = p_order_id
      AND COALESCE(moi.model_id_externo,'') = COALESCE(v_item_id,'')
    LIMIT 1;
    ...
    IF v_product_id IS NOT NULL THEN
      ...
      IF FOUND THEN
        ...
        INSERT INTO public.inventory_transactions (
          organizations_id, company_id, product_id, storage_id, pack_id, movement_type, quantity_change, timestamp, source_ref, order_id
        ) VALUES (
          v_org_id, v_company_id, v_product_id, p_storage_id, v_pack_id, 'RESERVA', (-1) * v_qty, now(), v_source_ref, p_order_id
        );
      END IF;
    END IF;
  END LOOP;
    ...
    IF v_product_id IS NOT NULL THEN
      ...
      IF FOUND THEN
        ...
        INSERT INTO public.inventory_transactions (
          ...
          v_org_id, v_company_id, v_product_id, p_storage_id, v_pack_id, 'SAIDA', (-1) * v_qty, now(), v_source_ref, p_order_id
        );
      END IF;
    END IF;
  END LOOP;
    ...
    IF v_product_id IS NOT NULL THEN
      ...
      IF FOUND THEN
        ...
        INSERT INTO public.inventory_transactions (
          ...
          v_org_id, v_company_id, v_product_id, p_storage_id, v_pack_id, 'CANCELAMENTO_RESERVA', v_qty, now(), v_source_ref, p_order_id
        );
      END IF;
    END IF;
  END LOOP;
