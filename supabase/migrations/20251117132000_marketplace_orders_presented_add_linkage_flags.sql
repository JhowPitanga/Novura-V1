...
WITH base AS (
  ...
  SELECT
    ...
    COALESCE(SUM(
      CASE WHEN
        COALESCE(oi->'item'->>'seller_sku', oi->>'seller_sku', oi->>'sku', '') = ''
        AND NOT EXISTS (
          SELECT 1 FROM public.marketplace_item_product_links mipl
          WHERE mipl.organizations_id = b.organizations_id
  ...
  SELECT
    ...
    COALESCE(lf.unlinked_items_count,
      CASE WHEN COALESCE(first_item_json->'item'->>'id', first_item_json->>'id', '') <> '' THEN
        CASE WHEN COALESCE(first_item_json->'item'->>'seller_sku', first_item_json->>'seller_sku', '') = ''
          AND NOT EXISTS (
            SELECT 1 FROM public.marketplace_item_product_links mipl
            WHERE mipl.organizations_id = b.organizations_id
              AND mipl.marketplace_name = b.marketplace
