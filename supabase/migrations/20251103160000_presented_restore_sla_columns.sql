-- Restaura colunas SLA individuais na view marketplace_orders_presented
...
SELECT
  ...
  -- Coluna JSON combinando SLA e atrasos do primeiro envio (quando houver)
  jsonb_build_object(
    'sla_status', COALESCE(b.shipments->0->>'sla_status', NULL),
    'sla_service', COALESCE(b.shipments->0->>'sla_service', NULL),
    'sla_expected_date', COALESCE(b.shipments->0->>'sla_expected_date', NULL),
    'sla_last_updated', COALESCE(b.shipments->0->>'sla_last_updated', NULL),
    'delays', COALESCE(b.shipments->0->'delays', '[]'::jsonb)
  ) AS shipping_sla_delays,

  -- Colunas individuais (compatibilidade com frontend existente)
  COALESCE(b.shipments->0->>'sla_status', NULL) AS shipment_sla_status,
  COALESCE(b.shipments->0->>'sla_service', NULL) AS shipment_sla_service,
  COALESCE(b.shipments->0->>'sla_expected_date', NULL) AS shipment_sla_expected_date,
  COALESCE(b.shipments->0->>'sla_last_updated', NULL) AS shipment_sla_last_updated,
