-- Corrige extração de campos do destinatário e método de envio na view
-- marketplace_orders_presented, trocando receiver_address por destination.shipping_address
-- e shipping_method por lead_time.shipping_method, além de ajustar índices.

BEGIN;

-- Recriar a view com extrações corretas para destinatário
...
SELECT
  ...
  -- Campos derivados do DESTINATÁRIO (destination.shipping_address)
  COALESCE(
  ...
  -- Método de envio preferencial: lead_time.shipping_method.name
  COALESCE(
  ...
  -- Deadline estimado: lead_time.estimated_delivery_limit.date (fallbacks)
  COALESCE(
...
-- Ajustar índices por expressão no raw para refletir os novos caminhos
DROP INDEX IF EXISTS idx_mor_org_ship_city_name;
DROP INDEX IF EXISTS idx_mor_org_ship_state_name;
DROP INDEX IF EXISTS idx_mor_org_ship_state_uf;
DROP INDEX IF EXISTS idx_mor_org_shipment_status;
DROP INDEX IF EXISTS idx_mor_org_shipping_method_name;
DROP INDEX IF EXISTS idx_mor_org_estimated_delivery_limit_at;

CREATE INDEX IF NOT EXISTS idx_mor_org_ship_city_name
  ON public.marketplace_orders_raw (
    organizations_id,
    ((shipments->0->'destination'->'shipping_address'->'city'->>'name'))
  );

CREATE INDEX IF NOT EXISTS idx_mor_org_ship_state_name
  ON public.marketplace_orders_raw (
    organizations_id,
    ((shipments->0->'destination'->'shipping_address'->'state'->>'name'))
  );
...
CREATE INDEX IF NOT EXISTS idx_mor_org_ship_state_uf
  ON public.marketplace_orders_raw (
    ...
    (
      COALESCE(
        NULLIF(split_part(shipments->0->'destination'->'shipping_address'->'state'->>'id', '-', 2), ''),
...
-- Recriar índice para shipping_type (mesma expressão; mantido por compatibilidade)
DROP INDEX IF EXISTS idx_mor_org_shipping_type;
