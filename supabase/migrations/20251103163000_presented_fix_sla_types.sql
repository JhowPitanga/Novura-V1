-- Ajusta tipos das colunas SLA na view marketplace_orders_presented e mantém permalink do primeiro item
...
SELECT
  ...
  jsonb_build_object(
    'sla_status', COALESCE(b.shipments->0->>'sla_status', b.shipments->0->'sla'->>'status'),
    'sla_service', COALESCE(b.shipments->0->>'sla_service', b.shipments->0->'sla'->>'service'),
    'sla_expected_date', COALESCE(b.shipments->0->>'sla_expected_date', b.shipments->0->'sla'->>'expected_date'),
    'sla_last_updated', COALESCE(b.shipments->0->>'sla_last_updated', b.shipments->0->'sla'->>'last_updated'),
    'delays', COALESCE(b.shipments->0->'delays', '[]'::jsonb)
  ) AS shipping_sla_delays,

  -- Colunas individuais (compatibilidade com frontend existente, com tipos corretos)
